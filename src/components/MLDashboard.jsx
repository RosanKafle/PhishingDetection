...
const MLDashboard = () => {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const res = await axios.get('http://localhost:5001/api/analytics/ml/metrics', {
  ...
  const imgUrl = data?.image ? `http://localhost:5001/backend/data/ml_dashboard.png?t=${Date.now()}` : null;
